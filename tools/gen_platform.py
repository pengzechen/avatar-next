...
def write_platform_mk(conf, path):
    ...
    with open(path, 'w') as f:
        ...
        f.write(f"MEM_ROOTFS_BASE     := {conf.get('MEM_ROOTFS_BASE', '0x60000000')}\n")
        f.write(f"MEM_ROOTFS_SIZE     := {conf.get('MEM_ROOTFS_SIZE', '0x08000000')}\n")
        f.write(f"KERNEL_LINK_ADDR    := {conf.get('KERNEL_LINK_ADDR', '0xffff000040080000')}\n")
        ...
        f.write(f"DEV_MMIO_NEEDS_VMA  := {conf.get('DEV_MMIO_NEEDS_VMA', '0')}\n")
        f.write(f"DEV_UART_BASE_RAW   := {conf.get('DEV_UART_BASE', '0')}\n")
        f.write(f"DEV_UART_REG_SHIFT  := {conf.get('DEV_UART_REG_SHIFT', '0')}\n")
