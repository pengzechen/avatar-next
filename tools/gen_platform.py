#!/usr/bin/env python3
"""
gen_platform.py  —  从 platform.lua 生成 platform.mk 和内嵌 Lua 源码

用法:
  python3 tools/gen_platform.py <platform.lua> <build/platform.mk> <include/>
"""
...
def write_lua_blob_c(lua_path, build_dir):
    ...
    if not os.path.exists(lua_path):
        write_if_changed(out_path,
                         '/* No platform.lua found */\n'
                         'const char g_platform_lua_src[] = "";\n'
                         'const unsigned int g_platform_lua_src_len = 0u;\n')
        return
    ...
    lines = [
        '/* Auto-generated by tools/gen_platform.py — do not edit */\n',
        '/* Embedded platform.lua source (null-terminated byte array) */\n',
        'const char g_platform_lua_src[] = {\n',
    ]
    ...
    lines.append('};\n')
    lines.append(f'const unsigned int g_platform_lua_src_len = {len(data)}u;\n')
    write_if_changed(out_path, ''.join(lines))
...
def main():
    if len(sys.argv) < 4:
        print(f"Usage: {sys.argv[0]} <platform.lua> <build/platform.mk> <include/>")
        sys.exit(1)
