...
impl WifiClient {
    ...
    pub fn start_ap_open(
        ...
        self.teardown_vif(timeout_ms)?;
        log::info!("[WifiClient] AP stage: teardown complete");

        // ---- 基础 LMAC 配置 ----
        let rf_calib_timeout_ms = timeout_ms.max(30_000);
        send_rf_calib_req(&self.bus, chip, rf_calib_timeout_ms).map_err(WifiError::from)?;
        log::info!("[WifiClient] AP stage: RF calib complete");
        let mac = send_get_mac_addr_req(&self.bus, timeout_ms).map_err(WifiError::from)?;
        log::info!("[WifiClient] AP stage: MAC={:02x?}", mac);
        send_reset_req(&self.bus, timeout_ms).map_err(WifiError::from)?;
        log::info!("[WifiClient] AP stage: reset complete");
        send_me_config_req(&self.bus, chip, timeout_ms).map_err(WifiError::from)?;
        log::info!("[WifiClient] AP stage: ME config complete");
        send_me_chan_config_req(&self.bus, timeout_ms).map_err(WifiError::from)?;
        log::info!("[WifiClient] AP stage: channel config complete");
        ...
        send_mm_start_req(&self.bus, timeout_ms).map_err(WifiError::from)?;
        log::info!("[WifiClient] AP stage: MM start complete");
        ...
        log::info!("[WifiClient] AP stage: filter configured");
