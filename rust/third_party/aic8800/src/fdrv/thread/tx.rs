...
pub fn start(bus: Arc<WifiBus>) {
    ...
    crate::runtime::runtime().spawn_poll_task(
        ...
        alloc::boxed::Box::new(move |cx| {
            ...
            // 处理所有待发帧。无工作时不要进入 tx_process(),避免在未创建 VIF 前
            // 反复走 process_data_tx() 并刷屏/抢占等待 CFM 的任务。
            let did_work = if has_pending_work(&bus) {
                tx_process(&bus)
            } else {
                false
            };
...
fn perform_cmd_flow_control_and_send(
    ...
    log::debug!("[wifi-tx] CMD flow_ctrl={} len={}", fc_ok, send_len);

    let did_work = if fc_ok {
        match transport.write_fifo(transport.cmd_func(), transport.wr_fifo_addr(), cmd) {
            Ok(()) => {
                log::debug!(
                    "[wifi-tx] CMD write_fifo OK func={} addr=0x{:x} len={}",
                    transport.cmd_func(),
                    transport.wr_fifo_addr(),
                    send_len,
                );
                true
            }
...
fn process_data_tx(bus: &WifiBus) -> bool {
    ...
    // 未连接则清空队列
    if vif_idx == 0xFF {
        if bus.tx.pktcnt.load(Ordering::Acquire) > 0 {
