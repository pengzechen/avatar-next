...
pub fn start(bus: Arc<WifiBus>) {
    ...
    // SG2002 上 SDIO CARD_INT/PLIC 可能不可靠；D80 AP 启动阶段的 CFM 不能只靠
    // IRQ 唤醒。周期 kicker 只唤醒 RX 线程，不直接碰 SDIO，作为所有芯片的兜底轮询。
...
/// 主动轮询一次 RX FIFO。用于命令等待路径在 CARD_INT/worker 调度不可靠时
/// 同步捞取 CFM，避免响应滞留到超时后才被 RX worker 处理。
pub(crate) fn poll_once(bus: &WifiBus) {
    process_rx_frames(bus);
    bus.transport.unmask_card_irq();
    invoke_rx_data_callback();
}
...
fn process_cmd_rsp(bus: &WifiBus, msg_data: &[u8]) {
    ...
    if expected_cfm != 0 && msg_id == expected_cfm {
        ...
        log::debug!("[wifi-rx] rsp_queue len after push={}", queue.len());
        drop(queue);
