...
fn try_get_cfm_from_queue(
    ...
    while let Some(rsp) = queue.pop_front() {
        ...
        if msg.id == expected_cfm_id {
            log::debug!("[cmd_mgr] got cfm=0x{:04x} from rsp_queue", msg.id);
            let _ = redirected;
            drop(queue);
...
pub fn send_cmd_with_cfm_id(
    ...
    log::info!(
        "[cmd_mgr] send msg=0x{:04x} dest=0x{:04x} expect=0x{:04x} param_len={} timeout={}ms",
        msg_id,
        dest_id,
        expected_cfm_id,
        param.len(),
    ...
    let result = match runtime().block_until(Some(tout), &mut |cx| {
        let poll = poll_cfm_response(bus, expected_cfm_id, &mut cfm, cx);
        if poll.is_pending() {
            wait_polls = wait_polls.wrapping_add(1);
            crate::fdrv::thread::rx::poll_once(bus);
            let poll = poll_cfm_response(bus, expected_cfm_id, &mut cfm, cx);
            if poll.is_ready() {
                return poll;
            }
            if wait_polls.is_multiple_of(1000) {
                log::debug!(
                    "[cmd_mgr] waiting cfm=0x{:04x} polls={} queue_len={}",
                    expected_cfm_id,
                    wait_polls,
                    bus.cmd.rsp_queue.lock().len(),
                );
            }
            // Some boards do not deliver SDIO CARD_INT reliably during AP startup.
            // Kick RX so command CFMs are polled instead of depending on IRQ/waker delivery.
    ...
    match &result {
        Ok(cfm) => log::info!(
            "[cmd_mgr] done msg=0x{:04x} cfm=0x{:04x} len={}",
            msg_id,
            expected_cfm_id,
            cfm.len(),
        ),
        Err(e) => log::error!("[cmd_mgr] send_cmd 0x{:04x} error: {:?}", msg_id, e),
    }
