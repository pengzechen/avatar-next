...
impl CviSdhci {
    ...
    fn dump_state(&self, tag: &str) {
        log::info!(
            "[SDHCI][{}] arg=0x{:08x} cmd=0x{:04x} tm=0x{:04x} resp=0x{:08x} pres=0x{:08x}",
            tag,
            self.read::<u32>(SDHCI_ARGUMENT),
            self.read::<u16>(SDHCI_COMMAND),
            self.read::<u16>(SDHCI_TRANSFER_MODE),
            self.read::<u32>(SDHCI_RESPONSE),
            self.read::<u32>(SDHCI_PRESENT_STATE),
        );
        log::info!(
            "[SDHCI][{}] hc=0x{:02x} pwr=0x{:02x} clk=0x{:04x} timeout=0x{:02x} reset=0x{:02x} caps=0x{:08x} ver=0x{:04x}",
            tag,
            self.read::<u8>(SDHCI_HOST_CONTROL),
            self.read::<u8>(SDHCI_POWER_CONTROL),
            self.read::<u16>(SDHCI_CLOCK_CONTROL),
            self.read::<u8>(SDHCI_TIMEOUT_CONTROL),
            self.read::<u8>(SDHCI_SOFTWARE_RESET),
            self.read::<u32>(SDHCI_CAPABILITIES),
            self.read::<u16>(SDHCI_HOST_VERSION),
        );
        log::info!(
            "[SDHCI][{}] int: norm=0x{:04x} err=0x{:04x} n_sts_en=0x{:04x} e_sts_en=0x{:04x} n_sig=0x{:04x} e_sig=0x{:04x}",
            tag,
            self.read::<u16>(SDHCI_INT_STATUS_NORM),
            self.read::<u16>(SDHCI_INT_STATUS_ERR),
            self.read::<u16>(SDHCI_NORM_INT_STS_EN),
            self.read::<u16>(SDHCI_ERR_INT_STS_EN),
            self.read::<u16>(SDHCI_NORM_INT_SIG_EN),
            self.read::<u16>(SDHCI_ERR_INT_SIG_EN),
        );
        log::info!(
            "[SDHCI][{}] vendor: mshc=0x{:08x} phy_dly=0x{:08x} phy_cfg=0x{:08x}",
            tag,
            self.read::<u32>(VENDOR_MSHC_CTRL),
            self.read::<u32>(VENDOR_PHY_TX_RX_DLY),
            self.read::<u32>(VENDOR_PHY_CONFIG),
        );
    }
    ...
    fn poll_int_status(&self, bit: u16) -> Result<(), SdioError> {
        ...
        for _ in 0..1000 {
            ...
            if norm & NORM_INT_ERROR != 0 {
                ...
                self.dump_state("poll-error-fast");
    ...
    fn cmd53_xfer(
        ...
        log::trace!(
            "[SDHCI] CMD53 start func={} addr=0x{:05x} write={} inc={} block={} len={} arg=0x{:08x} bs={} nb={}",
            func,
            addr,
            write,
            inc_addr,
        ...
        if let Err(e) = self.wait_cmd_complete() {
            self.dump_state("cmd53-cmd-complete-error");
            return Err(e);
        }
        log::trace!("[SDHCI] CMD53 command complete");
    ...
    fn cmd53_write_fixed(
        ...
        log::trace!(
            "[SDHCI] CMD53 write_fixed begin func={} addr=0x{:05x} len={} blk_sz={} use_blk={}",
            func,
            addr,
            buf.len(),
            blk_sz,
        ...
        log::trace!("[SDHCI] CMD53 write_fixed xfer ready bs={} nb={}", bs, nb);
        self.pio_write(buf, bs, nb)?;
        log::trace!("[SDHCI] CMD53 write_fixed pio done");
        match self.wait_transfer_complete() {
            Ok(()) => {
                log::trace!("[SDHCI] CMD53 write_fixed transfer complete");
                Ok(())
            }
            Err(e) => {
                self.dump_state("cmd53-write-xfer-error");
                Err(e)
            }
        }
    }
    ...
    fn pio_write(&self, buf: &[u8], block_size: u16, nblocks: u16) -> Result<(), SdioError> {
        ...
        for _ in 0..nblocks {
            if let Err(e) = self.wait_buffer_write_ready() {
                self.dump_state("pio-write-bwr-error");
                return Err(e);
            }
            log::trace!("[SDHCI] PIO write block offset={} block_size={}", offset, block_size);
    ...
    /// CV181x/SG2002 vendor PHY routing for SDIO1.
    fn setup_vendor_phy(&self) {
        const PHY_TX_RX_DLY_DS_HS: u32 = 0x0100_0100;
        const PHY_CONFIG_DS_HS: u32 = 1;

        let vendor = self.read::<u32>(VENDOR_MSHC_CTRL);
        let next = vendor
            | VENDOR_MSHC_CTRL_FEEDBACK_CLK
            | VENDOR_MSHC_CTRL_TX_DLY_EN
            | VENDOR_MSHC_CTRL_RX_DLY_EN
            | VENDOR_MSHC_CTRL_SD1_SEL;
        self.write::<u32>(VENDOR_MSHC_CTRL, next);
        self.write::<u32>(VENDOR_PHY_TX_RX_DLY, PHY_TX_RX_DLY_DS_HS);
        self.write::<u32>(VENDOR_PHY_CONFIG, PHY_CONFIG_DS_HS);
        log::info!(
            "[SDIO] vendor PHY init: mshc=0x{:08x}->0x{:08x} phy_dly=0x{:08x} phy_cfg=0x{:08x}",
            vendor,
            next,
            self.read::<u32>(VENDOR_PHY_TX_RX_DLY),
            self.read::<u32>(VENDOR_PHY_CONFIG),
...
impl SdioHost for CviSdhci {
    fn init(&mut self) -> Result<(), SdioError> {
        ...
        // Step 1.6: SDIO1 vendor PHY/routing must be valid before the first CMD5.
        self.setup_vendor_phy();
