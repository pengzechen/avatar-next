...
pub fn sdio1_hw_init(cfg: &Sdio1HwConfig) {
    ...
    log::info!(
        "[SDIO1-HW] crg: clk_en0=0x{:08x} clk_byp0=0x{:08x} div_sd1=0x{:08x} div_100k=0x{:08x}",
        mmio_read::<u32>(cfg.crg_base_va + CLK_EN_0),
        mmio_read::<u32>(cfg.crg_base_va + CLK_BYP_0),
        mmio_read::<u32>(cfg.crg_base_va + DIV_CLK_SD1),
        mmio_read::<u32>(cfg.crg_base_va + DIV_CLK_100K_SD1),
    );
    log::info!(
        "[SDIO1-HW] rtc: rst=0x{:08x} clkmux=0x{:08x} clkbyp=0x{:08x} clk_en=0x{:08x}",
        mmio_read::<u32>(cfg.rtcsys_ctrl_base_va + RTCSYS_RST_CTRL),
        mmio_read::<u32>(cfg.rtcsys_ctrl_base_va + RTCSYS_CLKMUX),
        mmio_read::<u32>(cfg.rtcsys_ctrl_base_va + RTCSYS_CLKBYP),
        mmio_read::<u32>(cfg.rtcsys_ctrl_base_va + RTCSYS_CLK_EN),
    );
    log::info!(
        "[SDIO1-HW] sysctrl: sd_ctrl_opt=0x{:08x} fmux={:x}/{:x}/{:x}/{:x}/{:x}/{:x} rtcio_e4=0x{:08x}",
        mmio_read::<u32>(cfg.sysctrl_base_va + SD_CTRL_OPT),
        mmio_read::<u32>(fmux_base + 0xD0),
        mmio_read::<u32>(fmux_base + 0xD4),
        mmio_read::<u32>(fmux_base + 0xD8),
        mmio_read::<u32>(fmux_base + 0xDC),
        mmio_read::<u32>(fmux_base + 0xE0),
        mmio_read::<u32>(fmux_base + 0xE4),
        mmio_read::<u32>(cfg.rtcsys_io_base_va + FMUX_SD1_VO),
    );
}
