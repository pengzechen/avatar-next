...
`tests/klog_test.c` 里有级别/模块/动态切换的用例，但 `run_klog_tests()` **目前没有任何调用者**（和之前 `tests/string_test.c` 一样是死代码）。要跑起来需要在 `kernel_main` 里显式调用；字符串那套自检已经用 `make ... test-string` 接通了（见 `docs/STRING.md`），klog 这套还没接。
