...
//!    仅作用于 `[start, start+size)` 覆盖到的若干 64B 行，适合以太网 RX/TX ring。
//!
//! 2. **粗粒度 DMA 一致性**（[`dcache_clean_for_dma`] / [`dcache_invalidate_after_dma`]）：
//!    AArch64 走标准 `dc cvac/ivac`，C906 走全清 `dcache.ciall`。
//!
//! # 平台与 feature 矩阵
//!
//! | 目标             | feature `c906`  | 实际行为                                       |
//! |------------------|-----------------|-------------------------------------------------|
//! | `riscv64` SG2002 | **on（默认）**  | T-Head C906 非标指令 `dcache.cva/iva/ciall`     |
//! | `riscv64` 其它   | off             | 退化为 `fence iorw, iorw`                       |
//! | `aarch64`        | n/a             | 标准 `dc cvac/ivac` + `dsb sy`                  |
//! | 其它 ISA         | n/a             | 全部退化为空操作                                 |

// 与标准 RISC-V Zicbom 缓存指令冲突时给出明确报错
#[cfg(all(target_arch = "riscv64", target_feature = "zicbom", feature = "c906"))]
compile_error!(
    "feature `c906` 与 RISC-V `zicbom` 标准缓存指令冲突，请二选一。"
);
    ...
    while addr < end {
        ...
        unsafe {
            match op {
                ...
                DcacheOp::Invalidate => {
                    core::arch::asm!("dc ivac, {}", in(reg) a, options(nostack))
                }
            }
        }
...
/// T-Head C906：`dcache.ciall` 清洗并无效全部 D-Cache。
#[cfg(all(target_arch = "riscv64", feature = "c906"))]
#[inline(always)]
...
/// 非 C906 RISC-V 核：仅做 `fence rw, rw` 保证内存序。
#[cfg(all(target_arch = "riscv64", not(feature = "c906")))]
pub unsafe fn dcache_clean_for_dma(_ptr: *const u8, _len: usize) {
