...
fn visit_default_depth(
    ...
    if iface_class == USB_CLASS_VIDEO && st.extras.uvc.is_none() {
        ...
        topo_log!(depth, "[USB]   -> Video class device (UVC candidate) addr={}",
            fn_addr);
    } else if dev_class == USB_CLASS_IAD && iface_class == 0 && st.extras.uvc.is_none() {
        st.extras.uvc = Some(UvcEnumerated {
            addr: fn_addr,
            ep0_mps,
            vid,
        ...
        topo_log!(depth, "[USB]   -> IAD device (UVC fallback, ifc_class parse failed) addr={}",
            fn_addr);
    }
