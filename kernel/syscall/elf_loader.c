/*
 * kernel/syscall/elf_loader.c - ELF 程序加载器
 *
...
static int elf_load_segment(void *pgd, elf64_phdr_t *phdr, uint8_t *file_data)
{
    ...
    for (uint64_t cur_vaddr = vaddr_start; cur_vaddr < vaddr_end; cur_vaddr += PAGE_SIZE) {
        ...
        /* 映射页表 */
        if (memory_create_map(pgd, cur_vaddr, paddr, 1, perm) != 0) {
 ...
 */
static int
elf_setup_stack(pte_t *pgd, uint64_t stack_top, uint64_t entry,
                ...
                uint64_t phdr_uaddr, uint16_t phnum, uint16_t phent)
{
    ...
    uint64_t top_page_paddr  = memory_get_paddr(pgd, top_page_uvaddr);
    if (top_page_paddr == 0) {
                    ...
                    char **argv, char **envp)
{
    ...
    pgd = (pte_t *)phys_to_virt(pgd_phys);
    ...
    for (uint64_t vaddr = stack_bottom; vaddr < USER_STACK_ADDR; vaddr += PAGE_SIZE) {
        ...
        if (memory_create_map(pgd, vaddr, stack_page, 1, 0) != 0) {
