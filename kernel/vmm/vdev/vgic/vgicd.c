 ...
 *   - log::info → KLOG_INFO
 *   - 中断挂起/使能/active 状态由 vgic core 统一维护
 */
...
{
    vmm_vgic_set_pending(vcpu_id, irq);
}

int vgicd_is_enabled(uint32_t vcpu_id, uint32_t irq)
{
    if (irq >= VGICD_MAX_IRQS)
        return 0;
    return (vmm_vgic_enabled_word(vcpu_id, irq / 32) & (1u << (irq & 31))) != 0;
}
...
static void handle_sgir(uint32_t value, uint32_t source_vcpu)
{
    ...
    for (uint32_t cpu = 0; cpu < g_vgicd.nr_vcpus && cpu < 32; cpu++) {
        if (mask & (1u << cpu))
            vmm_vgic_set_sgi_pending(cpu, source_vcpu, irq);
    }
}
                                    ...
                                    uint8_t size, uint32_t vcpu_id)
{
    ...
    if (off == GICD_CTLR)
        return vmm_vgic_dist_enabled() ? 1 : 0;

    switch (off) {
    case GICD_TYPER:
        /* Keep the exposed line count small until full SPI emulation lands. */
    ...
    case GICD_ICENABLER:
        ...
        if (word_index(off, GICD_ISENABLER, &w) ||
            ...
            return vmm_vgic_enabled_word(vcpu_id, w);
        }
        return 0;
    default:
        break;
    ...
    if (word_index(off, GICD_ISPENDR, &w) || word_index(off, GICD_ICPENDR, &w))
        return vmm_vgic_pending_word(vcpu_id, w);

    if (word_index(off, GICD_ISACTIVER, &w) || word_index(off, GICD_ICACTIVER, &w))
        return vmm_vgic_active_word(vcpu_id, w);
                                 ...
                                 uint32_t vcpu_id)
{
    ...
    if (off == GICD_CTLR) {
        vmm_vgic_set_dist_enabled((v & 1u) != 0);
    ...
    if (word_index(off, GICD_ISENABLER, &w)) {
        for (uint32_t b = 0; b < 32; b++) {
            if (v & (1u << b))
                vmm_vgic_set_enabled(vcpu_id, w * 32 + b, 1);
        }
    ...
    if (word_index(off, GICD_ICENABLER, &w)) {
        for (uint32_t b = 0; b < 32; b++) {
            if (v & (1u << b))
                vmm_vgic_set_enabled(vcpu_id, w * 32 + b, 0);
        }
        if (w == 0 && (v & (1u << VGICD_VTIMER_IRQ)))
    ...
    if (word_index(off, GICD_ISPENDR, &w)) {
        for (uint32_t b = 0; b < 32; b++) {
            if (v & (1u << b)) {
                uint32_t irq = w * 32 + b;
                uint32_t mask = (irq < 32) ? (1u << vcpu_id) :
                                g_vgicd.targets[irq < VGICD_MAX_IRQS ? irq : 0];
                if (mask == 0)
                    mask = 1;
                for (uint32_t cpu = 0; cpu < g_vgicd.nr_vcpus && cpu < 32; cpu++) {
                    if (mask & (1u << cpu))
    ...
    if (word_index(off, GICD_ICPENDR, &w)) {
        vmm_vgic_clear_pending_word(vcpu_id, w, v);
        return;
    }

    if (word_index(off, GICD_ICACTIVER, &w)) {
        vmm_vgic_clear_active_word(vcpu_id, w, v);
        return;
    }

    if (off == GICD_SGIR) {
        handle_sgir(v, vcpu_id);
        return;
    }
    ...
    if (off >= GICD_ITARGETSR && off < GICD_ITARGETSR + VGICD_MAX_IRQS) {
        ...
        for (uint32_t i = 0; i < size && idx + i < VGICD_MAX_IRQS; i++)
            g_vgicd.targets[idx + i] = (uint8_t)(value >> (8 * i));
    }
